...
const Map = dynamic(() => import('@/components/TrackOrderMap'), { 
    ssr: false,
    loading: () => <div className="flex items-center justify-center h-full bg-gray-200 rounded-lg"><p>Loading Map...</p></div>
});
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            setOrderStatus(prevStatus => {
                if (eta < 35 && prevStatus < 1) return 1; // Packed
                if (eta < 30 && prevStatus < 2) return 2; // On the Way
                if (eta <= 0 && prevStatus < 3) return 3;  // Delivered
                return prevStatus;
            });
    ...
    return (
        <div className="bg-gray-50 min-h-screen">
            <div className="container mx-auto p-4 md:p-8">
                <Link href="/orders" className="inline-flex items-center gap-2 text-green-700 hover:text-green-900 mb-6 font-semibold">
                    <ArrowLeft size={18} />
                    Back to My Orders
                </Link>

                <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
                    <h1 className="text-2xl font-bold">Tracking Order #{params.id}</h1>
                    <p className="text-gray-500">Follow your fresh produce from the farm to your home.</p>
                        ...
                        {STATUSES.map((status, index) => (
                            <React.Fragment key={status}>
                                <div className="flex flex-col items-center text-center">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${index <= orderStatus ? 'bg-green-600 border-green-600 text-white' : 'bg-gray-100 border-gray-300'}`}>
                                        {index < orderStatus ? '✓' : index + 1}
                                    </div>
                                    <p className={`mt-2 font-semibold ${index <= orderStatus ? 'text-green-600' : 'text-gray-500'}`}>{status}</p>
                                </div>
                                {index < STATUSES.length - 1 && <div className={`flex-1 h-1 mx-2 ${index < orderStatus ? 'bg-green-600' : 'bg-gray-300'}`}></div>}
                            </React.Fragment>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-1 bg-white rounded-xl shadow-lg p-6 flex flex-col justify-between">
                        <div>
                            ...
                            <div className="my-6 text-center">
                                ...
                                {eta > 0 ? (
                                    <p className="text-5xl font-extrabold text-green-600">{eta} <span className="text-3xl">min</span></p>
                                ) : (
                                    <p className="text-4xl font-extrabold text-green-600">Arrived</p>
                                )}
                            </div>
                        </div>
